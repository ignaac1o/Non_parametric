...
**b. Test the implementation by estimating the regression function in the location model $Y = m(X) + \epsilon$, where $m(x) = (x - 1)^2$, $X \sim N (1, 1)$, and $\epsilon \sim N (0, 0.5)$. Do it for a sample of size $\texttt{n} = 500$.**
...
Now, we had to implement the main function for the Euler solution. Here, we had some constraints that stopped the iteration if  $\|x_{t+1} - x_{t}\|_{\infty} < \epsilon \hspace{0.2cm} or \hspace{0.2cm} \|x_{t+1} - x_{t}\|_{\infty}/\|x_{t}\|_{\infty} < \epsilon$, but we added another constraint that disregarded the final points if $f(x) < 50*\delta$. For this, we implemented this constraint before plotting the final points, allowing to plot the points that only satisfy this constraint.  
...
# Projected gradient into the Hessian s-th eigenvector subspace
proj_grad_norm_crois <- function(x, mu, Sigma, s = 2) {
...
for (i in 1:nrow(x0)) {
  ...
  for (t in 1:N) {
    ...
    # Euler update
      phi[t + 1, ] <- phi[t, ] + 
        h * proj_grad_norm_crois(phi[t, ], mu = mu, Sigma = Sigma) /
        f_crois(x = phi[t, ])
...
```{r, include=FALSE}
# Projected gradient into the Hessian s-th eigenvector subspace
proj_grad_norm_sin <- function(x, mu, Sigma, s = 2) {
  
  # Gradient
  grad <- numDeriv::grad(f_sin,x) %>% as.matrix() %>% t()
  
  # Hessian
  Hess <- numDeriv::hessian(f_sin,x)
...
for (i in 1:nrow(x0)) {
  ...
  for (t in 1:N) {
    ...
    # Euler update
      phi[t + 1, ] <- phi[t, ] + 
        h * proj_grad_norm_sin(phi[t, ], mu = mu, Sigma = Sigma) /
        f_sin(x = phi[t, ])
